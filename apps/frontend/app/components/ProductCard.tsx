...
const ProductCard = ({
  ...
  return (
    <Link
      href={`/product/${id}`}
      className="group max-h-[364px] relative block shadow-lg overflow-hidden cursor-pointer min-w-[230px] max-w-[500px]"
    >
      ...
      <div className="absolute inset-0 flex flex-col items-end justify-between ">
        <button className="group/button m-4  relative inline-block overflow-hidden border bg-black border-black px-4 lg:px-7 py-1 lg:py-2 focus:outline-none focus:ring">
          <span className="absolute inset-y-0 left-0 w-[2px] bg-white transition-all group-hover/button:w-full group-active:bg-indigo-500"></span>

          <span className="relative text-xs lg:text-sm font-medium text-white transition-colors group-hover/button:text-black">
            Ajouter au panier
          </span>
        </button>
        <div className="flex flex-col w-full items-start backdrop-blur-md bg-black/20 p-1">
          <div className="flex justify-between items-center w-full px-2">
            <h3 className="text-sm xl:text-xl font-medium text-white">
              {name}
            </h3>
            <p className="text-white text-lg rounded-xl ">{price}€</p>
          </div>
