import {
  Carousel,
  CarouselContent,
  CarouselItem,
  CarouselNext,
  CarouselPrevious,
} from '@/components/ui/carousel'
import Image from 'next/image'
import Link from 'next/link'
import { ProductSchemaType } from '../schema/products.schema'
import { FaCartPlus } from "react-icons/fa";

const ProductCard = ({ product }: ProductSchemaType) => {
  return (
    // <Link href={`/product/${product._id}`}>
    <div className="h-72  relative flex flex-col justify-between shadow-lg w-64 overflow-hidden rounded-xl">
      <Carousel className="group">
        <CarouselContent>
          {product.imageArray?.map((image, index) => (
            <CarouselItem key={index}>
              <div 
              // href={`/product/${product._id}`}
              >
                <Image
                  src={image.path}
                  alt={image.name}
                  width={200}
                  height={200}
                  quality={50}
                  loading="lazy"
                  placeholder="blur"
                  blurDataURL="data:image/webp;base64,"
                  className="w-full hover:scale-110 transition-all duration-200"
                />
              </div>
            </CarouselItem>
          ))}
        </CarouselContent>
        <div className="absolute top-0 right-0">
          <button className="group/button rounded-bl-xl rounded-tr-xl m-2 relative inline-block overflow-hidden border bg-primary-foreground border-primary-foreground px-3 lg:px-4 py-2 focus:outline-none focus:ring">
            <div className="absolute flex justify-center items-center inset-y-0 left-0 w-[2px] bg-accent/50 transition-all group-hover/button:w-full ">
              <span className='opacity-0 text-2xl  group-hover/button:opacity-100'><FaCartPlus />  </span>
            </div>
            <span className="relative text-sm font-medium transition-colors group-hover/button:opacity-0">
              Ajouter au panier
            </span>
          </button>
        </div>

        <CarouselPrevious className="absolute top-3/4 left-2 z-50 bg-white" />
        <CarouselNext className="absolute top-3/4 right-2 z-50 bg-white" />
      </Carousel>
      <Link
        href={`/product/${product._id}`}
        className="absolute bottom-0 group/button flex flex-col w-full justify-center items-center h-14 p-2 !no-underline backdrop-blur-md bg-black/50 transition-all hover/button:bg-black/10 "
      >
        <div className="flex justify-center h-full">
          <div className="absolute inset-y-0 left-0 w-[2px] bg-white/50 transition-all delay-100 duration-200 group-hover/button:w-full group-active:bg-accent">
            <span className="absolute w-full h-full flex justify-center items-center opacity-0 transition-all delay-100 duration-200 group-hover/button:opacity-100 underline underline-offset-2">
              En savoir plus
            </span>
          </div>
          <span className="h-full w-60 text-sm mt-0.5 text-white text-center transition-all group-hover/button:opacity-0 overflow-hidden whitespace-nowrap text-ellipsis">
            {product.name}
          </span>
        </div>
        <span className="text-white  text-sm z-10 transition-all group-hover/button:opacity-0">
          {`${product.price} €`}
        </span>
      </Link>
    </div>

    // </Link>
  )
}

export default ProductCard
