...
const AdminProducts = async () => {
  ...
  return (
    <div className="flex flex-col p-6 bg-primary-foreground rounded-3xl">
      {/* {Filtre Les Categories } */}
      {arrayOfCategory.map((category, index) => (
        <div className="categoryFilter mb-10" key={'div' + index}>
          <h2 className="categoryTitle text-xl p-4 border-b border-dotted  border-black my-4">
            {category[0].toUpperCase() +
              category.slice(1, category.length).replaceAll('_', ' ')}
          </h2>
          <div
            key={index}
            className="gridOfProduct flex flex-wrap justify-around gap-6 items-center w-full"
          >
